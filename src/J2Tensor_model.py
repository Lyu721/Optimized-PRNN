...
class J2Material:
    def __init__(self,E=0.3130,nu_=0.3,A=64.8,B=33.6,C=0.003407, **kwargs):
        self.dev = kwargs.get('device',torch.device('cpu'))
        self.E = E*9000.0+1000.0
        self.nu_ = nu_*0.40+0.05
        self.A = A*40.+B*60.
        self.B = B*60.
        self.C = C*0.0099+0.0001
    ...
    def configure(self, npoints):
        ...
        self.epspeq_hist = torch.zeros((npoints), device=self.dev) 
        self.new_epsp_hist = torch.zeros((npoints, 3), device=self.dev)
        self.new_epspeq_hist = torch.zeros((npoints), device=self.dev) 
    ...
    def sigma_C(self, x):  # TODO include look-up table external file
        ...
        return self.A - self.B * torch.exp(x / -self.C)


    def sigma_C_deriv(self, x):
        # return 9862.048723216907*torch.exp(x/-0.003407)
        return self.B/self.C*torch.exp(x/-self.C)
...
class MyModel(nn.Module):
    def __init__(self, E0, nu0, A0, B0, C0):
        super(MyModel, self).__init__()
        self.E = nn.Parameter(torch.ones(1)*E0)
        self.nu = nn.Parameter(torch.ones(1)*nu0)
        self.A = nn.Parameter(torch.ones(1)*A0)
        self.B = nn.Parameter(torch.ones(1)*B0)
        self.C = nn.Parameter(torch.ones(1)*C0)
        self.J2 = J2Material(E=self.E, nu_=self.nu, A=self.A, B=self.B, C=self.C)
    def forward(self, x):
        ...
        for t in range(seq_len):
            ...
            outputt = self.J2.update(
                        epsilon_t.view(ip_pointsb,3))
            self.J2.commit() 
            sigma[:, t, :] = outputt.view(-1,3) 
        return sigma
